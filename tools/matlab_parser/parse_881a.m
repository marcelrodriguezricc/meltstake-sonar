...
for i = 1:ns
    ...
    % Check for angle match after flipping
    assert(all(all(abs(anglei_cw - anglei_ccw) <= tol)), ...
    'CW/CCW angle axes do not align after flipping')

    % Recombine angles
    angle_combined = [anglei_ccw; anglei_cw];

    % Check for angle match with previous scan
    if ~isempty(prev_angle_combined)
        assert(isequal(angle_combined, prev_angle_combined), ...
            'combined angle axes do not align between scans');
    end

    % Set current scan to be previous scan for next iteration
    prev_angle_combined = angle_combined;

    % Append to array with unified angle, cw and ccw data, for all scans
    angle_data(i, :) = angle_combined;
    scan_data_ccw(i,:,:) = scani_ccw;
    scan_data_cw(i,:,:) = scani_cw;
end
